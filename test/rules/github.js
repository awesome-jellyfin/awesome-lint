...
test.serial.failing("github - error invalid git repo", async (t) => {
	...
	const messages = await lint({
		config,
		filename: "test/fixtures/github/0.md",
	});
...
test.serial.failing(
	"github - repo without description and license",
	async (t) => {
		const execaStub = sandbox.stub(github.execa, "stdout");
...
test.serial.failing(
	"github - API rate limit exceeded with token",
	async (t) => {
		const execaStub = sandbox.stub(github.execa, "stdout");
		const gotStub = sandbox.stub(github.got, "get");
...
test.serial.failing(
	"github - API rate limit exceeded without token",
	async (t) => {
		const execaStub = sandbox.stub(github.execa, "stdout");
		const gotStub = sandbox.stub(github.got, "get");
